    ...
    for name, values in report["application_examples"].items():
        ...
        lines.extend(
            [
                ...
                f"- Maximum possible manufacturing overlap: `{manufacturing['max_overlap']:.6g}`.",
                f"- Inspection coverage reliable in the declared example: `{inspection['reliable']}`.",
                f"- Maximum possible inspection coverage gap: `{inspection['max_coverage_gap']:.6g}`.",
                "",
            ]
        )
